...
function BookReader() {
    ...
    this.logoURL = 'https://digitalna.milutinbojic.rs';
...
BookReader.prototype.prepareTwoPagePopUp = function() {
    ...
    $(this.leafEdgeR).bind('mousemove', this, function(e) {
        ...
        var jumpIndex = e.data.jumpIndexForRightEdgePageX(e.pageX);
        $(e.data.twoPagePopUp).text('Погледај ' + e.data.getPageName(jumpIndex));

        // $$$ TODO: Make sure popup is positioned so that it is in view
        // (https://bugs.edge.launchpad.net/gnubook/+bug/327456)
        $(e.data.twoPagePopUp).css({
            left: e.pageX- $('#BRcontainer').offset().left + $('#BRcontainer').scrollLeft() - 110 + 'px',
            top: e.pageY - $('#BRcontainer').offset().top + $('#BRcontainer').scrollTop() - 2 + 'px'
        });
    });
    ...
    $(this.leafEdgeL).bind('mousemove', this, function(e) {
        ...
        //           (https://bugs.edge.launchpad.net/gnubook/+bug/327456)
        $(e.data.twoPagePopUp).css({
            left: e.pageX - $('#BRcontainer').offset().left + $('#BRcontainer').scrollLeft() - $(e.data.twoPagePopUp).width() + 110 + 'px',
            top: e.pageY-$('#BRcontainer').offset().top + $('#BRcontainer').scrollTop() - 2 + 'px'
        });
    });
}
...
BookReader.prototype.blankInfoDiv = function() {
    return $([
        '<div class="BRfloat" id="BRinfo">',
            '<div class="BRfloatHead">О публикацији',
...
BookReader.prototype.getPageName = function(index) {
    return 'Стр. ' + this.getPageNum(index);
}
...
BookReader.prototype.buildShareDiv = function(jShareDiv)
{
    ...
    var jForm = $([
        '<p>Опције за дељење публикације</p>',
        '<form method="post" action="">',
            '<fieldset>',
                '<label for="pageview">Веза према прегледу ове стране публикације</label>',
                '<input type="text" name="pageview" id="pageview" value="' + pageView + '"/>',
            '</fieldset>',
            '<fieldset>',
                '<label for="booklink">Веза према публикацији:</label>',
                '<input type="text" name="booklink" id="booklink" value="' + bookView + '"/>',
            '</fieldset>',
            '<fieldset>',
                '<label for="iframe">Угради мини читач:</label>',
                '<fieldset class="sub">',
                    '<label class="sub">',
            ...
            '<fieldset class="center">',
                '<button type="button" onclick="$.fn.colorbox.close();">Настави</button>',
            '</fieldset>',
        '</form>'].join('\n'));
...
BookReader.prototype.initUIStrings = function()
{
    ...
    var titles = { '.logo': 'Дигитална библиотека', // $$$ update after getting OL record
                   '.zoom_in': 'Увећај',
                   '.zoom_out': 'Умањи',
                   '.onepg': 'Преглед једне стране',
                   '.twopg': 'Преглед две стране',
                   '.thumb': 'Преглед по странама',
                   '.print': 'Штампај ову страну',
                   '.embed': 'Угради читач',
                   '.link': 'Веза до ове публикације (и стране)',
                   '.bookmark': 'Означи ову страну',
                   '.read': 'Гласовно читање',
                   '.share': 'Подели',
                   '.info': 'О публикацији',
                   '.full': 'Приказ пуног екрана',
                   '.book_left': 'Предходна страна',
                   '.book_right': 'Следећа страна',
                   '.book_up': 'Предходна страна',
                   '.book_down': 'Следећа страна',
                   '.play': 'Листај',
                   '.pause': 'Заустави',
                   '.BRdn': 'Сакриј/Прикажи', // Would have to keep updating on state change to have just "Hide nav bar"
                   '.BRup': 'Сакриј/Прикажи',
                   '.book_top': 'Прва страна',
                   '.book_bottom': 'Последња страна'
                  };
    if ('rl' == this.pageProgression) {
        titles['.book_leftmost'] = 'Последња страна';
        titles['.book_rightmost'] = 'Прва страна';
    } else { // LTR
        titles['.book_leftmost'] = 'Прва страна';
        titles['.book_rightmost'] = 'Последња страна';
    }
